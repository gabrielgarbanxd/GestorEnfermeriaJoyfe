...
-- //===>> GetAllCalendarTasksPaginatedProcedure calendar tasks procedure <<===//
DROP PROCEDURE IF EXISTS `GetAllCalendarTasksPaginatedProcedure`;

DELIMITER $$

CREATE PROCEDURE `GetAllCalendarTasksPaginatedProcedure`(
    In p_per_page INT,
    In p_page INT,
    OUT p_Result INT
)
PRO : BEGIN
    ...
    -- Obtener el número total de registros
    SELECT COUNT(*) INTO @total_records FROM calendar;
